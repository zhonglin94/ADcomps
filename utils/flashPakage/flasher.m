...
%}
    ...
    wi  =   mix.W;         % Acentric Factors
     % Use Wilson equation to initialize vapor/liquid equilibrium constant
    ...
    elseif alpha<0
        ...
        % We assign the same PVT properties to oil and gas when the
        % mixture is in single phase region
        Z = [ZL ZL]; 
        alpha = eps;
        ki = ones(1, numel(zi)).*eps; % Avoid dividing by zeros
     elseif alpha>1
        st = 2; % Vapor like
        zi = num2cell(zi,1);
