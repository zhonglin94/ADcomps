...
[p, sO, sG, xc, yc, zc, st, wellSol] = model.getProps(state, ...
    'pressure', 'oil', 'gas', 'xc', 'yc', 'zc', 'status', 'wellSol');
% Properties at previous timestep
[p0, sO0, sG0, xc0, yc0, rhom0, wellSol0] = model.getProps(state0, ...
    'pressure', 'oil', 'gas', 'xc', 'yc','rhom', 'wellSol');
...
% Initialize primary variabes, include fluid compositions
 [p, xc{1:end-1},sO, yc{1:end-1}, sG, wellVars{:}] = initVariablesADI(p,  xc{1:end-1}, sO, yc{1:end-1}, sG, wellVars{:}); 
tempx =0; tempy =0;
for i = 1:nc-1
    tempx =tempx+xc{:,i};
    tempy =tempy+yc{:,i};
...
names = {'pressure', xcVars{1:end-1}, 'so', ycVars{1:end-1}, 'sg'};
